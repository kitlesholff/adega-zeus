...
const CACHE_PREFIX = 'adega-zeus-installation-';
const CACHE_NAME = `${CACHE_PREFIX}v2`;
const base = self.registration.scope;
const offlineUrl = new URL('offline.html', base).href;
const offlineAssets = [offlineUrl, new URL('assets/pwa/zeus.png', base).href];
  ...
  if (request.mode === 'navigate') {
    event.respondWith((async () => {
      try {
        ...
        const cache = await caches.open(CACHE_NAME);
        return await cache.match(offlineUrl) || new Response('Sem conexão. Conecte-se à internet e abra a Adega Zeus novamente.', {
