(function () {
  ...
  function reportMarkup(row) {
    ...
    return `<div class="closing-report-heading"><h3>${dateLabel(row.closing_date)} · versão ${row.revision}</h3><p>${escape(row.responsible)} · registrado em ${dateTime(row.created_at)} · Extrema/MG</p></div>
  ...
  $('#printCashClosing').addEventListener('click', () => {
    ...
    if (!popup) { window.AdminFeedback?.notify('Permita a abertura de janelas para imprimir ou salvar o PDF.', 'warning'); return; }
    popup.opener = null;
    popup.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Adega Zeus — Caixa ${escape(report.closing_date)} v${report.revision}</title><style>
      ...
      </style></head><body><header><strong>ADEGA &amp; TABACARIA ZEUS</strong><h1>Fechamento de caixa</h1></header><div class="print-actions"><button id="printReport" type="button">Imprimir / Salvar como PDF</button></div>${reportMarkup(report)}</body></html>`);
