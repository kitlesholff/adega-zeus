(function () {
  ...
  function exportSpreadsheet(data) {
    ...
    const rows = [
      ['RELATÓRIO GERAL — ADEGA & TABACARIA ZEUS'],
    ...
    download(csv, 'text/csv;charset=utf-8', `adega-zeus-controle-geral-${new Date().toISOString().slice(0, 10)}.csv`);
  }
  ...
  function exportPdf(data, reportWindow) {
    ...
    reportWindow.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Relatório geral — Adega Zeus</title><style>
    ...
    </style></head><body><header><div><strong>ADEGA &amp; TABACARIA ZEUS</strong><h1>Relatório geral</h1><p>Pedidos, movimentação financeira e catálogo</p></div><div>Gerado em ${escapeHtml(dateTime(new Date()))}</div></header>
    ...
    <footer>Documento gerado pelo painel administrativo da Adega &amp; Tabacaria Zeus.</footer></body></html>`);
