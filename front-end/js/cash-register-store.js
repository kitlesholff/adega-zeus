  ...
  async function rpc(action, payload = {}) {
    ...
    const run = () => {
      ...
      if (action === 'order_status') {
        ...
        if (order.status === 'cancelled') throw Error('Pedido cancelado não pode ser alterado.');
        ...
        if (action === 'movement') {
          ...
          const savedSnapshot = JSON.parse(JSON.stringify(fresh)); delete savedSnapshot.latest; savedSnapshot.closing_basis = 'all_payments';
          db.closings.push({ id: crypto.randomUUID(), session_id: session.id, business_date: session.business_date, revision: payload.revision + 1, ...values, expected_cash: summary.balance, responsible, created_at: now, snapshot: savedSnapshot });
