-- Execute após 10-rotina-diaria-reset.sql. Não apaga dados ou comprovantes.
begin;
...
begin
  if tg_op <> 'INSERT' and old.status in ('confirmed','cancelled') then
    raise exception 'Pedido confirmado ou cancelado não pode ser alterado ou excluído. Registre a devolução como saída de caixa.';
  end if;
...
begin
  if tg_op<>'INSERT' and exists(select 1 from public.orders where id=old.order_id and status in ('confirmed','cancelled')) then
    raise exception 'Itens de pedido confirmado ou cancelado não podem ser alterados.';
  end if;
  if tg_op<>'DELETE' and exists(select 1 from public.orders where id=new.order_id and status in ('confirmed','cancelled')) then
    raise exception 'Itens de pedido confirmado ou cancelado não podem ser alterados.';
  end if;
  if tg_op='DELETE' then return old; end if;
  return new;
...
begin
  ...
  if o.status in ('confirmed','cancelled') then raise exception 'Pedido confirmado ou cancelado não pode ser alterado. Registre a devolução como saída de caixa.'; end if;
