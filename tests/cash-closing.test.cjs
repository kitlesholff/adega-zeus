...
test('dia de Extrema: meia-noite local respeita o fuso de São Paulo', () => {
  assert.equal(core.dayKey('2020-03-11T02:59:59Z'), day);
  assert.equal(core.dayKey('2020-03-11T03:00:00Z'), '2020-03-11');
  const s = core.snapshot(day, [order('a', 10, 'Pix', 'confirmed', '2020-03-11T02:59:59Z'), order('b', 20, 'Pix', 'confirmed', '2020-03-11T03:00:00Z')], []);
...
test('armazenamento isolado preserva versões e recusa dados alterados ou fechamento duplicado', async () => {
  ...
  vm.runInNewContext(fs.readFileSync(require.resolve('../front-end/js/cash-closing-store.js'), 'utf8'), sandbox);
