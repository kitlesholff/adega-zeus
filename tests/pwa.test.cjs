...
function worker({ offline = false } = {}) {
  ...
  const sandbox = {
    ...
    caches: {
      ...
      keys: async () => ['adega-zeus-installation-v0', 'adega-zeus-installation-v1', 'other-app-cache'],
...
test('instalação guarda somente a página offline e a logo; atualização respeita outros caches', async () => {
  ...
  assert.deepEqual(w.calls, ['https://snoop.test/offline.html', 'https://snoop.test/assets/pwa/zeus.png']);
  w.events.activate({ waitUntil: promise => { done = promise; } }); await done;
  assert.deepEqual(w.deleted, ['adega-zeus-installation-v0', 'adega-zeus-installation-v1']);
});
