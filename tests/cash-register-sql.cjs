...
(async()=>{
 ...
 try {
  await db.exec(`create role anon; create role authenticated; create schema auth; create table auth.users(id uuid primary key);
    ...
    grant usage on schema auth to anon,authenticated;insert into auth.users values('${admin}');set test.user_id='${admin}';`);
  for(const file of ['schema.sql','01-restringir-admin.sql']) await db.exec(fs.readFileSync('back-end/supabase/'+file,'utf8').replace(/create extension if not exists pgcrypto;/g,''));
  await db.exec(`insert into private.admin_users(user_id) values('${admin}')`);
  for(const file of ['02-criar-saidas.sql','06-fechamento-caixa.sql','08-abertura-fechamento.sql','08-abertura-fechamento.sql']) await db.exec(fs.readFileSync('back-end/supabase/'+file,'utf8').replace(/create extension if not exists pgcrypto;/g,''));
