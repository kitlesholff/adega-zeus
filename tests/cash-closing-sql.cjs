...
const core = require('../front-end/js/cash-closing-core.js');
...
(async () => {
  ...
  try {
    ...
    await db.exec(read('back-end/supabase/schema.sql').replace(/create extension if not exists pgcrypto;/i, ''));
    await db.exec(read('back-end/supabase/01-restringir-admin.sql'));
    await db.exec(`insert into private.admin_users(user_id) values ('${admin}')`);
    await db.exec(read('back-end/supabase/02-criar-saidas.sql').replace(/create extension if not exists pgcrypto;/i, ''));
    const migration = read('back-end/supabase/06-fechamento-caixa.sql');
    ...
    await db.exec(`insert into public.orders(code,customer_name,delivery_type,address,payment,client_total,trusted_total,status,created_at) values
      ...
      ('C1','Cliente C','Retirada','Venda presencial','Cartão de crédito',80,80,'confirmed','2020-03-11T02:59:59Z'),
      ('X1','Cliente D','Entrega','Rua A','Pix',25,25,'cancelled','2020-03-10T15:00:00Z'),
      ('NEXT','Cliente E','Entrega','Rua A','Pix',900,900,'confirmed','2020-03-11T03:00:00Z');
    ...
    await db.exec(read('back-end/supabase/05-controle-geral.sql'));
